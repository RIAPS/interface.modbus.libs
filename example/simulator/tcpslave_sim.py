...
required_parameters = {
    "modbus_block": ["block_type",
                     "starting_address",
                     "num_registers"],
    "device": ["register_name",
               "block_name",
               "starting_address",
               "initial_values",
               "num_registers"]
...
def main(port, address, modbus_config_file_path, device_config_file_path):
    ...
    for block_type_name in modbus_block_config:
        ...
        slave1.add_block(block_name=block_type_name,
    ...
    if verbose:
        print(f'{Yellow}{"Initialization info":<56}'
              f': {"Variable Name":<30}'
              f'{"Address":<14}'
              f'{"Order":<8}'
              f'{"Value":<10}'
    ...
    for parameter_name in device_conifg:
        ...
        if verbose:
            print(f'Initializing {f"{format}{len(intermediate_bytes)*8}":<8}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--address', required=False, help="IP Address. Default:127.0.0.1", default="127.0.0.1")
    parser.add_argument('-p', '--port', required=False, help="Modbus TCP Port (1-65535). Default:501", default=501, type=int)
    parser.add_argument('-c', '--cfg', required=False, help="Path to modbus config file", default="modbus_cfg.yaml", type=str)
    parser.add_argument('-d', '--dcfg', required=False, help="Path to device config file", default="device_cfg.yaml", type=str)

    args = parser.parse_args()
    server = main(port=args.port, address=args.address, modbus_config_file_path=args.cfg, device_config_file_path=args.dcfg)
    server.start()
    print("Started")
